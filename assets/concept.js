...
(function () {
  ...
  /* 📌 **되살아난 화면에서 창을 닫지 않는다** (2026-08-11, 사용자 확인).
     「섹션 전체 보기」로 나갔다가 뒤로가면 **창이 열렸던 그 자리로 돌아와야 한다** —
     이 기능이 애초에 그러려고 있는 것이다.

     🚨 T38 에서 한 번 닫게 만들었다가 되돌렸다. 스크롤 잠금이 남을까 걱정했는데,
        bfcache 는 DOM 과 리스너를 통째로 보존한다 — 창이 열려 있으니 잠긴 것이 맞고,
        Esc·바깥 클릭도 그대로 살아 있어 닫으면 함께 풀린다.
        새로 연 경우(persisted=false)는 페이지가 처음부터라 창 자체가 없다. */
