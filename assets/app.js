...
(function () {
  ...
  /* -------------------------------------------------------------- 기록 이관 */
  /* 2026-08-07: 출제기준 세부항목 순서에 맞춰 ch02·ch11 의 섹션을 재배치했다.
     섹션 번호가 바뀌었으므로 이미 쌓인 기록을 새 번호로 옮겨 준다.

       eip.done.chNN   ["s01.html", …]        진도
       eip.quiz.chNN   { "1": {score,…}, … }  섹션별 점수
       eip.wrong.all   { "ch02-s30-01": … }   오답노트
       eip.fav.all     { "ch02-s30-01": … }   저장한 문제

     🚨 한 번만 돌아야 한다. 두 번 돌면 이미 옮긴 것을 또 옮겨 뒤섞인다.
        그래서 완료 표시(mig.sec1)를 남긴다. */
  (function migrateSectionOrder() {
    if (store.get('mig.sec1', false)) return;

    /* 옛 번호 → 새 번호 */
    var MAP = {
      ch02: { 30: 1, 31: 2, 32: 3, 1: 30, 2: 31, 3: 32 },
      ch11: { 14: 5, 5: 6, 13: 7, 15: 8, 6: 9, 7: 10, 8: 11, 9: 12, 10: 13, 11: 14, 12: 15 }
    };

    function moved(ch, sec) {
      var m = MAP[ch];
      return (m && m[sec]) ? m[sec] : sec;
    }
    function pad(n) { return (n < 10 ? '0' : '') + n; }

    var chapters = ['ch02', 'ch11'], i, ch;

    for (i = 0; i < chapters.length; i++) {
      ch = chapters[i];

      /* 진도 — 파일명으로 저장돼 있다 */
      var done = store.get('done.' + ch, null);
      if (done && done.length) {
        var newDone = [], j, mm;
        for (j = 0; j < done.length; j++) {
          mm = /^s(\d\d)\.html$/.exec(done[j]);
          newDone.push(mm ? ('s' + pad(moved(ch, parseInt(mm[1], 10))) + '.html') : done[j]);
        }
        store.set('done.' + ch, newDone);
      }

      /* 섹션별 점수 — 키가 섹션 번호 문자열이다 */
      var quiz = store.get('quiz.' + ch, null), k, newQuiz;
      if (quiz) {
        newQuiz = {};
        for (k in quiz) {
          if (Object.prototype.hasOwnProperty.call(quiz, k)) {
            newQuiz[String(moved(ch, parseInt(k, 10)))] = quiz[k];
          }
        }
        store.set('quiz.' + ch, newQuiz);
      }
    }

    /* 오답노트·저장한 문제 — 문항 id 안에 섹션 번호가 들어 있다 */
    var idKeys = ['wrong.all', 'fav.all'], n;
    for (n = 0; n < idKeys.length; n++) {
      var src = store.get(idKeys[n], null), out, id, hit;
      if (!src) continue;
      out = {};
      for (id in src) {
        if (!Object.prototype.hasOwnProperty.call(src, id)) continue;
        hit = /^(ch\d\d)-s(\d\d)-(\d\d)$/.exec(id);
        if (hit && MAP[hit[1]]) {
          out[hit[1] + '-s' + pad(moved(hit[1], parseInt(hit[2], 10))) + '-' + hit[3]] = src[id];
        } else {
          out[id] = src[id];
        }
      }
      store.set(idKeys[n], out);
    }

    store.set('mig.sec1', true);
  })();
